...
def fetch_current():
  ...
  for i in range(0,1):                          # ****** MAIN FOR-LOOP *******
      ...
      url = f"https://api.weatherbit.io/v2.0/history/hourly?lat=8.26&lon=77.56&start_date={past_day}&end_date={current_date}&tz=local&key={API_KEY}"
...
# Historical DATA
df_prev = pd.read_csv("/home/ec2-user/files/Muppandal.csv",index_col=[0])
...
# Future Fetch
url = f"https://api.weatherbit.io/v2.0/forecast/hourly?lat=8.26&lon=77.56&key={API_KEY}&hours=120"
...
df_future.to_csv("Muppandal_future.csv")
...
df_jsons = df_jsons.T
df_jsons.to_json('Muppandal_Wind_48hrs.json', orient='records')
...
df_feature_json[['Humidity','Pressure','Temperature','Precipitation','UV Index','Ozone',
                 'Rain_Probability','Clouds']] = df_feature_json[['Humidity','Pressure','Temperature','Precipitation','UV Index','Ozone',
                                                         'Rain_Probability','Clouds']].apply(pd.to_numeric)
...
double_df.to_json('Muppandal_Features_2_days.json', orient='records')
...
XGB_result.to_csv("Muppandal_Predictions_Ady.csv")
